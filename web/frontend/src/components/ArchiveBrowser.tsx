...
import { archiveAPI, crackAPI } from '@/lib/api';
...
export default function ArchiveBrowser({
  ...
  const [batchFrames, setBatchFrames] = useState<any[]>([]);
  const [batchPage, setBatchPage] = useState(1);
  const [batchTotal, setBatchTotal] = useState(0);
  const [batchLoading, setBatchLoading] = useState(false);
  const batchPageSize = 200;
  ...
  const handleFrameAction = async (
    ...
    try {
      if (batchResultIndex !== undefined) {
        setBatchFrames(prev => prev.map(frame =>
          frame._batch_result_index === batchResultIndex
      ...
      await archiveAPI.postAction('/snapshot/action', {
        ...
        status: newStatus,
        batch_result_index: batchResultIndex
      });
    } catch (e) {
      console.error(e);
    }
  ...
  const loadBatchPage = async (page: number, taskId = selectedTaskData?.task_id) => {
    if (!taskId) return;
    setBatchLoading(true);
    try {
      const { data } = await crackAPI.getBatchResults(taskId, page, batchPageSize, true);
      setBatchFrames(data?.items || []);
      setBatchTotal(Number(data?.total || 0));
      setBatchPage(Number(data?.page || page));
    } catch (e) {
      console.error('Failed to fetch paginated batch results', e);
      setBatchFrames([]);
      setBatchTotal(0);
    } finally {
  ...
  useEffect(() => {
    ...
    if (selectedTaskData?.task_id) {
      void loadBatchPage(1, selectedTaskData.task_id);
    }
  }, [selectedTaskData?.task_id]);

  const reviewFrames = batchTotal > 0
    ? batchFrames
    : (selectedTaskData?.best_frames || []);
  const reviewTaskData = selectedTaskData
    ? { ...selectedTaskData, best_frames: reviewFrames }
    : null;
